...
function slotToMarkee(slot: MarkeeSlot): Markee {
  return { address: slot.address, message: slot.message, owner: slot.owner, totalFundsAdded: slot.totalFundsAdded, chainId: 8453, pricingStrategy: '' }
}
...
function MarkeeRowSkeleton({ rank }: { rank: number }) {
  const mw = ['w-4/5', 'w-3/4', 'w-2/3', 'w-5/6', 'w-1/2'][rank % 5]
  return (
...
export default function WebsiteLeaderboardPage() {
  ...
  useEffect(() => {
    fetch(`/api/openinternet/meta?address=${leaderboardAddress}`)
      .then(r => r.json())
      .then(setSiteMeta)
      .catch(() => {})
  }, [leaderboardAddress])
  ...
  const { data: meta, isLoading: isMetaLoading, refetch: refetchMeta } = useReadContracts({ contracts: metaContracts })
  ...
  const refetch = useCallback(() => { refetchMeta(); refetchDetails() }, [refetchMeta, refetchDetails])
  ...
  const displayUrl = siteMeta?.verifiedUrl ?? siteMeta?.siteUrl
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
            <div className="flex items-center gap-5">
              <div className="flex items-center justify-center w-14 h-14 rounded-2xl bg-[#0A0F3D] border border-[#8A8FBF]/30 overflow-hidden flex-shrink-0">
                {siteMeta?.logoUrl ? (
                  <img src={siteMeta.logoUrl} alt={leaderboardName} className="w-10 h-10 object-contain" />
                ) : (
                  <Globe2 size={28} className="text-[#F897FE]" />
                )}
              </div>
              <div>
                <div className="flex items-center gap-3 mb-1 flex-wrap">
                  ...
                  {displayUrl && (
                    <a
                      ...
                      className="flex items-center gap-1 text-[#8A8FBF] hover:text-[#F897FE] text-xs transition-colors"
                    >
          ...
          {isMetaLoading ? (
            ...
            <div className="flex flex-wrap items-center gap-8 mt-8">
              ...
              {minimumPrice !== undefined && minimumPrice > 0n && (
                <div className="flex items-center gap-2 text-sm">
                  <Globe2 size={14} className="text-[#8A8FBF]" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {isLoading ? (
            <div className="space-y-3">
              {[1, 2, 3, 4, 5].map(i => <MarkeeRowSkeleton key={i} rank={i} />)}
            </div>
          ) : markees.length === 0 ? (
...
function MarkeeRow({
  markee, rank, formatFunds, trackView, viewCount, onAddFunds, onEditMessage,
}: {
  markee: MarkeeSlot
  rank: number
  ...
  useEffect(() => {
    if (markee.message) trackView(slotToMarkee(markee))
