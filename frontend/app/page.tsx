...
export default function Home() {
  ...
  return (
    <>
      <InvestmentModal 
        isOpen={isModalOpen || isInvestmentModalOpen}
        onClose={closeModals}
        markee={selectedMarkee}
      ...
      <div className="min-h-screen bg-[#060A2A]">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
            <div className="flex items-center gap-8">
              ...
              <nav className="hidden md:flex gap-6">
                <Link href="/how-it-works" className="text-[#8A8FBF] hover:text-[#EDEEFF] transition-colors">How it Works</Link>
                <Link href="/ecosystem" className="text-[#8A8FBF] hover:text-[#EDEEFF] transition-colors">Ecosystem</Link>
                <Link href="/owners" className="text-[#8A8FBF] hover:text-[#EDEEFF] transition-colors">Owners</Link>
              </nav>
            </div>
            <ConnectButton />
          </div>
        ...
        {/* Hero */}
        <section className="bg-gradient-to-b from-[#0A0F3D] to-[#060A2A] py-12 border-b border-[#8A8FBF]/20">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h1 className="text-4xl md:text-5xl font-bold text-[#EDEEFF] mb-4">
              Invest in <span className="text-[#F897FE]">Open Source Marketing</span>
            </h1>
            <p className="text-lg md:text-xl text-[#8A8FBF] mb-6">
              Every message funds websites, DAOs, communities, open source, and more.
            </p>
        ...
        {/* Fixed Price Markees */}
        <section className="bg-[#060A2A] py-12 border-b border-[#8A8FBF]/20">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl font-bold text-[#EDEEFF] mb-8 text-center">Readerboards 📟</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {isLoadingFixed && (
                Array.from({ length: 3 }).map((_, i) => (
                  <div key={i} className="bg-[#0A0F3D] rounded-lg p-6 border border-[#8A8FBF]/20 animate-pulse">
                    <div className="h-32 bg-[#8A8FBF]/20 rounded mb-4"></div>
                    <div className="h-4 bg-[#8A8FBF]/20 rounded mb-2"></div>
                    <div className="h-4 bg-[#8A8FBF]/20 rounded w-2/3"></div>
                  </div>
                ))
              )}

              {!isLoadingFixed && fixedMarkees.map((fixedMarkee) => (
                <button
                  key={fixedMarkee.address}
                  onClick={() => handleChangeFixedMarkee(fixedMarkee)}
                  className="group text-left"
                >
                  <div className="readerboard-card">
                  ...
                  <div className="mt-4 text-center">
                    <div className="text-[#EDEEFF] font-semibold mb-1">{fixedMarkee.name}</div>
                    <div className="text-[#8A8FBF] text-sm">
        ...
        <section className="bg-[#060A2A] py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {markees.length > 0 && (
              <div className={isFetchingFresh ? 'opacity-70' : ''}>
                {/* #1 Hero Card */}
                {markees.length > 0 && (
                  <MarkeeCard 
                    markee={markees[0]} 
                ...
                {markees.length > 3 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
                    {markees.slice(3, 27).map((markee, index) => (
                      <MarkeeCard 
                        key={markee.address} 
                        markee={markee} 
                        rank={index + 4} 
